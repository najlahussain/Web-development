...
let x : string = array.join('-');
console.log(`Join  array: ${array} and new array is: ${x}`);

let y : number[] = array.slice(5,8);
...
let z : boolean = array.includes(33);
...
let z1: boolean[] = array.map((n)=> n % 2 === 0);
...
result = array.at(-2);//counts from the end starting index from 1
console.log(`Result of some function is: ${result}`);

result = array.at(2);//counts from the beginning starting at 0 index
console.log(`Result of some function is: ${result}`);