...
while(running){
  ...
  let enemyHealth: number = parseInt(temp.toFixed());
  let enemy : string = enemies[parseInt((rand * enemies.length).toFixed())];//generating a random index number
  ...
  while(enemyHealth > 0) {
    ...
    if(answer.choice==1) {
      let damageDealt : number = parseInt((rand * attackDamage).toFixed());
      let damageTaken : number = parseInt((rand * enemyAttackDamage).toFixed());
